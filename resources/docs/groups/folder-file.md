...
```bash
curl -X GET \
    -G "http://localhost/api/folder_files/eaque" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
...
```javascript
const url = new URL(
    "http://localhost/api/folder_files/eaque"
);
...
```bash
curl -X DELETE \
    "http://localhost/api/folder_files/rerum" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
