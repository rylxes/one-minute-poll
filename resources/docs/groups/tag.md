...
curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"name":"exercitationem","colour":"sunt","company_id":6}'
...
```bash
curl -X PUT \
    "http://localhost/api/tags/assumenda" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"name":"accusamus","colour":"sit","company_id":16}'
