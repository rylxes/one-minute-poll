...
curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"name":"ab","colour":"aut","company_id":15}'
...
```bash
curl -X PUT \
    "http://localhost/api/tags/rerum" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"name":"voluptas","colour":"aspernatur","company_id":16}'
...
```bash
curl -X DELETE \
    "http://localhost/api/tags/tempore" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
