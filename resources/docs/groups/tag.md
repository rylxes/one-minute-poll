...
curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"name":"quam","colour":"vel","company_id":3}'
...
```bash
curl -X PUT \
    "http://localhost/api/tags/odit" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"name":"eveniet","colour":"libero","company_id":12}'
