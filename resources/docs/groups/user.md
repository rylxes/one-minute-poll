...
curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"group_id":"veritatis","email":"soluta"}'
...
```bash
curl -X GET \
    -G "http://localhost/api/users/est" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
...
```bash
curl -X DELETE \
    "http://localhost/api/users/explicabo" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
